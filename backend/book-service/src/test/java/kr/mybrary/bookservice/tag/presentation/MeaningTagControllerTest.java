...
class MeaningTagControllerTest {
    ...
    void getPageMeaningTags() throws Exception {
        ...
        actions
                ...
                .andExpect(jsonPath("$.message").value("의미 태그 페이징 조회 결과입니다."))
                .andExpect(jsonPath("$.data").isNotEmpty());

        // document
        actions.andDo(document("find-paging-meaning-tags",
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()),
                resource(
                        ResourceSnippetParameters.builder()
                                .tag("meaningTag")
                                .summary("의미 태그 페이징 조회한다.")
                                .description("등록된 수가 많은 순으로 의미 태그가 조회된다. size 값에 따라 조회되는 수가 달라지며, size는 1보다 항상 커야 한다.")
                                .queryParameters(
                                ...
                                .responseSchema(Schema.schema("find_paging_meaning_tags_response_body"))
                                .responseFields(
